                               ...
                               android_thread_id_t *threadId)
{
    ...
    int result = pthread_create(&thread, &attr,
                    (android_pthread_entry)entryFunction, userData);//创建线程
...
status_t Thread::run(const char* name, int32_t priority, size_t stack)
{
    ...
    if (mCanCallJava) {
        res = createThreadEtc(_threadLoop,
                this, name, priority, stack, &mThread);//最终也是调用androidCreateRawThreadEtc方法
    } else {
...
{
    Thread* const self = static_cast<Thread*>(user);//当前Thread实例
    ...
    do {
        ...
        if (first) {
            ...
            if (result && !self->exitPending()) {
                ...
                result = self->threadLoop();//调用threadLoop方法，该方法由子类实现
            }
        } else {
